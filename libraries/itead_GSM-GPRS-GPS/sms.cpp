...
#if 1 // add by wpf
SMSGSM::SMSGSM(SIMCOM900 &gsm_): _gsm(&gsm_)
{
}
#endif
...
char SMSGSM::SendSMS(char *number_str, char *message_str)
{
     if(strlen(message_str)>159)
          printf(F("Don't send message longer than 160 characters\n"));
     char ret_val = -1;
     byte i;
     char end[2];
     end[0]=0x1a;
     ...
     */
     ...
     for (i = 0; i < 1; i++) {
          ...
          __gsm.flush();/* add by wpf Clear buffer for response */
          __gsm.SimpleWrite("AT+CMGS=\"");
          __gsm.SimpleWrite(number_str);
          __gsm.SimpleWriteln("\"");
          ...
          if (RX_FINISHED_STR_RECV == __gsm.WaitResp(1000, 500, ">")) {
#ifdef DEBUG_ON
               printf("DEBUG:>\n");
#endif
               // send SMS text
               __gsm.flush(); // add by wpf
               __gsm.SimpleWrite(message_str);
               __gsm.SimpleWriteln(end);
               if (RX_FINISHED_STR_RECV == __gsm.WaitResp(7000, 5000, "+CMGS")) {
...
char SMSGSM::SendSMS(byte sim_phonebook_position, char *message_str)
{
     ...
     // ret_val = 0; // SMS is not send yet // comment by wpf
     if (sim_phonebook_position == 0) return (-3);
     if (1 == __gsm.GetPhoneNumber(sim_phonebook_position, sim_phone_number)) {
...
char SMSGSM::IsSMSPresent(byte required_status)
{
     ...
     if (CLS_FREE != __gsm.GetCommLineStatus()) return (ret_val);
     __gsm.SetCommLineStatus(CLS_ATCMD);
     ret_val = 0; // still not present
     __gsm.flush(); // add by wpf
     switch (required_status) {
     case SMS_UNREAD:
          __gsm.SimpleWriteln(F("AT+CMGL=\"REC UNREAD\""));
          break;
     case SMS_READ:
          __gsm.SimpleWriteln(F("AT+CMGL=\"REC READ\""));
          break;
     case SMS_ALL:
          __gsm.SimpleWriteln(F("AT+CMGL=\"ALL\""));
          break;
     }
     ...
     case RX_FINISHED:
          ...
          // ---------------------------------------------
          if(__gsm.IsStringReceived("+CMGL:")) {
...
char SMSGSM::GetSMS(byte position, char *phone_number, char *SMS_text, byte max_SMS_len)
{
     ...
     __gsm.SimpleWrite(F("AT+CMGR="));
     __gsm.SimpleWriteln((int)position);

     // 5000 msec. for initial comm tmout
     // 1000 msec. for inter character tmout
     switch (__gsm.WaitResp(5000, 1000, "+CMGR")) {
     case RX_TMOUT_ERR:
          // response was not received in specific time
          ret_val = -2;
          break;
     ...
     case RX_FINISHED_STR_NOT_RECV:
          ...
          } else if(__gsm.IsStringReceived("ERROR")) {
     ...
     case RX_FINISHED_STR_RECV:
          ...
          if(__gsm.IsStringReceived("\"REC UNREAD\"")) {
          ...
          else if(__gsm.IsStringReceived("\"REC READ\"")) {
...
char SMSGSM::DeleteSMS(byte position)
{
     ...
     __gsm.SimpleWrite(F("AT+CMGD="));
     __gsm.SimpleWriteln((int)position);


     // 5000 msec. for initial comm tmout
     // 20 msec. for inter character timeout
     switch (__gsm.WaitResp(5000, 50, "OK")) {
     case RX_TMOUT_ERR:
          // response was not received in specific time
          ret_val = -2;
          break;
