...
#if 1 // add by wpf
GPSGSM::GPSGSM(SIMCOM900 & gsm_):_gsm(&gsm_)
{
}
#endif
...
char GPSGSM::getBattInf(char *str_perc, char *str_vol)
{
     ...
     __gsm.SimpleWriteln("AT+CBC");
     __gsm.WaitResp(5000, 100, "OK");
     if(__gsm.IsStringReceived("+CBC"))
          ret_val=1;
...
char GPSGSM::getBattTVol(char *str_vol)
{
    ...
    __gsm.flush(); // add by wpf
     __gsm.SimpleWriteln("AT+CBTE?");
     __gsm.WaitResp(5000, 100, "OK");
     if(__gsm.IsStringReceived("+CBTE"))
          ret_val=1;
...
{
     if(AT_RESP_ERR_DIF_RESP == __gsm.SendATCmdWaitResp("AT+CGPSPWR=1", 500, 100, "OK", 5))
          return 0;
     if(AT_RESP_ERR_DIF_RESP == __gsm.SendATCmdWaitResp("AT+CGPSRST=1", 500, 100, "OK", 5))
          return 0;
     return 1;
}
...
{
     if(AT_RESP_ERR_DIF_RESP == __gsm.SendATCmdWaitResp("AT+CGPSPWR=0", 500, 100, "OK", 5))
          return 0;
     return 1;
}
...
char GPSGSM::getStat()
{
     char ret_val=-1;
     __gsm.flush(); // add by wpf
     __gsm.SimpleWriteln("AT+CGPSSTATUS?");
     __gsm.WaitResp(5000, 100, "OK");
     if(__gsm.IsStringReceived("Unknown")||__gsm.IsStringReceived("unknown"))
          ret_val=0;
     else if(__gsm.IsStringReceived("Not"))
          ret_val=1;
     else if(__gsm.IsStringReceived("2D")||__gsm.IsStringReceived("2d"))
          ret_val=2;
     else if(__gsm.IsStringReceived("3D")||__gsm.IsStringReceived("3d"))
          ret_val=3;
     return ret_val;
}
...
char GPSGSM::getPar(char *str_long, char *str_lat, char *str_alt, char *str_time, char *str_speed)
{
     ...
     char *p_char1;
     __gsm.flush(); // add by wpf
     __gsm.SimpleWriteln("AT+CGPSINF=0");
     #if 0 // add by wpf
     /* Here! Receiving is very delay,or Segment fault */
     __gsm.WaitResp(5000, 100, "OK");
     #else
     __gsm.WaitResp(8000,1000, "OK");
     #endif
