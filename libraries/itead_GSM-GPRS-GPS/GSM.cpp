...
#if 0
GSM::GSM():_cell(DEV_UART2),_tf(_cell, 10),_status(IDLE)
{
}
#endif

#if 1
GSM::GSM(uint16_t gsm_on,Serial_ &Serial):_pcell(&Serial),_tf(_cell, 10),_status(IDLE),GSM_ON(gsm_on)
{
    GSM_RST = PIN_NONE;
}
GSM::GSM(uint16_t gsm_on, uint16_t gsm_rst, Serial_ &Serial):_pcell(&Serial),_tf(_cell, 10),_status(IDLE),GSM_ON(gsm_on)
{
    GSM_RST = gsm_rst;
}
...
int GSM::begin(long baud_rate)
{
     ...
     if( GSM_RST != PIN_NONE ) {
        if(pinMode(GSM_RST,OUTPUT)) {
            printf("Invalid GSM_RST pin = %d\n",GSM_RST);
        }
     } else {
        //printf("Invalid GSM_RST pin = %d\n",GSM_RST);
     }
     ...
     _cell.begin(baud_rate); /* set baud rate for communication */
     p_comm_buf = &comm_buf[0];
     setStatus(IDLE);

     // if no-reply we turn to turn on the module
     // After {for} below, GSM must be ON POWER. // add by wpf
...
#ifdef DEBUG_ON
          printf(F("DB:CORRECT BR\n"));
#endif
          turnedON=true;
          norep=false;
     }

    /* If now GSM has not been ON right baud, It should be wrong about baud rate.
        * Try any possible baud to connect to GSM module.
        */
     if (AT_RESP_ERR_DIF_RESP == SendATCmdWaitResp(str_at, 500, 100, str_ok, 5)&&!turnedON) {		//check OK
#ifdef DEBUG_ON
          printf(F("DB:AUTO BAUD RATE\n"));
#endif
          for (int i=0; i<8; i++) {
               switch (i) {
               case 0:
     ...
     if(norep==true&&!turnedON) {
          printf(F("Trying to force the baud-rate to 9600\n"));
          for (int i=0; i<8; i++) {
               switch (i) {
               case 0:
                    _cell.begin(1200);
          ...
          printf(F("ERROR: SIM900 doesn't answer. Check power and serial pins in GSM.cpp\n"));
          digitalWrite(GSM_ON, HIGH);
          delay(1200);
          digitalWrite(GSM_ON, LOW);
          delay(10000);
...
void GSM::RxInit(uint16_t start_comm_tmout, uint16_t max_interchar_tmout)
{
     ...
     p_comm_buf = &comm_buf[0];
     comm_buf_len = 0;
     //_cell.flush(); // erase rx circular buffer // comment by wpf
}
