...
#endif
#if 1 // add by wpf
SIMCOM900::SIMCOM900(uint16_t gsm_on, Serial_ &Serial):GSM(gsm_on,Serial)
{
}

SIMCOM900::SIMCOM900(uint16_t gsm_on, uint16_t gsm_rst, Serial_ &Serial): GSM(gsm_on,gsm_rst,Serial)
{
}
...
#if 1 // add by wpf
void SIMCOM900::powerOn()
{
    printf("powerOn: Do nothing actually!\n");
}
void SIMCOM900::powerDown()
{
    printf("powerDown: Do nothing actually!\n");
}
void SIMCOM900::powerReset()
{
    printf("powerReset: Do nothing actually!\n");
}
#endif
...
int SIMCOM900::configandwait(char* pin)
{
     ...
     for(int i=0; i<10; i++) {
          ...
          while(WaitResp(5000, 50, "+CGREG: 0,")!=RX_FINISHED_STR_RECV)
...
#ifdef DEBUG_ON
     printf(F("Starting read..\nWaiting for Data.."));
#endif
     // Wait until we start receiving data
     while(_cell.available()<1) {
          delay(100);
#ifdef DEBUG_ON
          printf(F("."));
#endif
...
{
     printf(F("This method is deprecated! Please use GetSMS in the SMS class.\n"));
     long index;
     char *p_char;
     char *p_char1;
