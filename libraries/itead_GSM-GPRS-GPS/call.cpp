...
#if 1 // add by wpf
CallGSM::CallGSM(SIMCOM900 & gsm_):_gsm(&gsm_)
{
}
#endif
...
byte CallGSM::CallStatus(void)
{
     ...
     if (CLS_FREE != __gsm.GetCommLineStatus()) return (CALL_COMM_LINE_BUSY);
     __gsm.SetCommLineStatus(CLS_ATCMD);
     __gsm.flush(); // add by wpf
     __gsm.SimpleWriteln(F("AT+CPAS"));

     // 5 sec. for initial comm tmout
     // 50 msec. for inter character timeout
     if (RX_TMOUT_ERR == __gsm.WaitResp(5000, 50)) {
          ...
          if(__gsm.IsStringReceived("+CPAS: 0")) {
               // ready - there is no call
               // ------------------------
               ret_val = CALL_NONE;
          } else if(__gsm.IsStringReceived("+CPAS: 3")) {
               // incoming call
               // --------------
               ret_val = CALL_INCOM_VOICE;
          } else if(__gsm.IsStringReceived("+CPAS: 4")) {
               // active call
               // -----------
     ...
     __gsm.SetCommLineStatus(CLS_FREE);
     return (ret_val);
                                 ...
                                 byte first_authorized_pos, byte last_authorized_pos)
{
     ...
     __gsm.SetCommLineStatus(CLS_ATCMD);
     __gsm.flush(); // add by wpf
     __gsm.SimpleWriteln(F("AT+CLCC"));

     // 5 sec. for initial comm tmout
     // and max. 1500 msec. for inter character timeout
     __gsm.RxInit(5000, 1500);
     ...
     if (status == RX_FINISHED) {
          ...
          if(__gsm.IsStringReceived("+CLCC: 1,1,4,0,0")) {
               ...
               ret_val = CALL_OTHERS;
          } else if(__gsm.IsStringReceived("OK")) {
          ...
          if (search_phone_num) {
               ...
               // ---------------------------
               p_char = strchr((char *)(__gsm.comm_buf),'"');
               ...
               if ( (ret_val == CALL_INCOM_VOICE_NOT_AUTH)
                    ...
                    if ((first_authorized_pos == 0) && (last_authorized_pos == 0)) {
                         ...
                         for (i = first_authorized_pos; i <= last_authorized_pos; i++) {
                              if (__gsm.ComparePhoneNumber(i, phone_number)) {
...
void CallGSM::PickUp(void)
{
     ...
     __gsm.SendATCmdWaitResp("ATA", 10, 10, "OK", 3);
     __gsm.SimpleWriteln("ATA");
...
void CallGSM::HangUp(void)
{
     //if (CLS_FREE != gsm.GetCommLineStatus()) return;
     //gsm.SetCommLineStatus(CLS_ATCMD);
     __gsm.SendATCmdWaitResp("ATH", 500, 100, "OK", 5);
     //gsm.SetCommLineStatus(CLS_FREE);
}
...
{
     if (CLS_FREE != __gsm.GetCommLineStatus()) return;
     __gsm.SetCommLineStatus(CLS_ATCMD);
     // ATDxxxxxx;<CR>
     __gsm.flush(); // add by wpf
     __gsm.SimpleWrite(F("ATD"));
     __gsm.SimpleWrite(number_string);
     __gsm.SimpleWriteln(F(";"));
     // 10 sec. for initial comm tmout
     // 50 msec. for inter character timeout
     __gsm.WaitResp(10000, 50);
     __gsm.SetCommLineStatus(CLS_FREE);
}
...
void CallGSM::Call(int sim_position)
{
     ...
     __gsm.flush(); // add by wpf
     // ATD>"SM" 1;<CR>
     __gsm.SimpleWrite(F("ATD>\"SM\" "));
     __gsm.SimpleWrite(sim_position);
     __gsm.SimpleWriteln(F(";"));
...
void CallGSM::SendDTMF(char *number_string, int time)
{
     ...
     __gsm.flush(); // add by wpf
     __gsm.SimpleWrite(F("AT+VTD="));
     __gsm.SimpleWriteln(time);
     __gsm.WaitResp(1000, 100, "OK");


     __gsm.flush(); // add by wpf
     __gsm.SimpleWrite(F("AT+VTS=\""));
     __gsm.SimpleWrite(number_string);
     __gsm.SimpleWriteln(F("\""));
...
{
     if(DTMF_status==1)
          __gsm.SendATCmdWaitResp("AT+DDET=1", 500, 50, "OK", 5);
     else
          __gsm.SendATCmdWaitResp("AT+DDET=0", 500, 50, "OK", 5);
}
...
char CallGSM::DetDTMF()
{
     ...
     char *p_char1;
     char dtmf_char='-';
     __gsm.WaitResp(1000, 500);
     {
          p_char = strstr((char *)(__gsm.comm_buf),"+DTMF:");
          if (p_char != NULL) {
